  ...
  return (
    <article className={`overflow-hidden rounded-xl border bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg ${mine ? "border-2 border-rose-500 bg-rose-50/30" : top3 ? "border-2 border-amber-400" : "border-slate-200"}`}>
      <a href={link} target="_blank" rel="noreferrer" className={`relative flex items-center justify-center bg-white p-2 ${compact ? "h-44" : "h-72"}`}>
        ...
        </> : <div className="flex h-[90%] w-[90%] flex-col items-center justify-center rounded-xl border-2 border-dashed border-rose-200 bg-rose-50 text-xs font-semibold text-rose-500"><span className="text-3xl">▧</span>暂无商品主图</div>}
        ...
        {product.status && <span className={`absolute right-2 top-2 rounded-full border-2 px-3 py-1.5 text-sm font-black shadow-sm ${product.status.includes("下降") || product.status.includes("跌出") || product.status.startsWith("↓") ? "border-green-300 bg-green-50 text-green-700" : "border-rose-300 bg-rose-50 text-rose-700"}`}>{product.status}</span>}
      </a>
      <div className="p-3">
      ...
      <footer className="flex justify-between border-t border-slate-100 px-3 py-2 text-[11px] text-slate-400"><span>点击主图查看详情</span><a href={link} target="_blank" rel="noreferrer" className="font-semibold text-rose-600">天猫详情 →</a></footer>
    </article>
  );
}
