  ...
  return (
    <div className="space-y-5">
      ...
      <Panel title="雨虹产品生命周期" subtitle="退出=上周在Top100、本周明确出现跌出Top100信号；仍有当前排名的商品不会判为退出">
        ...
        <div key={lifecycleStage} className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{stageProducts.map((p)=>{const track=p.weeks.map((week,i)=>({week,rank:p.rank[i]})).filter(x=>x.week<=w).slice(-3).map(x=>x.rank);return <ProductInsightCard key={p.id} product={{productId:p.id,name:p.name,brand:p.brand,category:p.category,rank:p.cur,track,status:lifecycleStage,judgment:lifecycleStage==="退出"?`第${w-1}周排名 ${track.at(-1)??"—"}，第${w}周无Top100排名且收到明确跌出信号。`:lifecycleRules[lifecycleStage]}} />})}{stageProducts.length===0&&<div className="col-span-full rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 p-8 text-center text-base font-bold text-slate-500">当前阶段暂无商品</div>}</div>
      </Panel>

      <Panel title="雨虹上升产品" subtitle="商品卡展示"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{riseProducts.map((a)=><ProductInsightCard key={`${a.productId}-${a.type}`} product={{productId:a.productId,name:a.name,brand:a.brand,category:a.category,direction:a.direction,rank:a.currentRank,track:a.track,status:a.type,visitors:a.visitors,conversion:a.conversion,judgment:a.judgment,action:a.action}} />)}</div></Panel>
      <Panel title="雨虹风险产品" subtitle="商品卡展示"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{riskProducts.map((a)=><ProductInsightCard key={`${a.productId}-${a.type}`} product={{productId:a.productId,name:a.name,brand:a.brand,category:a.category,direction:a.direction,rank:a.currentRank,track:a.track,status:a.type,visitors:a.visitors,conversion:a.conversion,judgment:a.judgment,action:a.action}} />)}</div></Panel>
    </div>
  );
}
