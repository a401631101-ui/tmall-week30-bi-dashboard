...
export function MarketCategory(){
  ...
  return <div className="space-y-4">
    ...
    <Panel title={`${active} · 本周 Top10 商品`} subtitle="官方主图、品牌、排名和趋势集中展示"><div className="grid gap-5 md:grid-cols-3 xl:grid-cols-5">{detail.top10.slice(0,10).map(p=><ProductInsightCard key={p.productId} compact product={p}/>)}</div></Panel>
    <Panel title="重点异动商品" subtitle="机会与风险信号直接落到商品"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{anomalies.map(({type,item})=><ProductInsightCard key={`${type}-${item.productId}`} compact product={{productId:item.productId,name:item.name,brand:item.brand,category:active,rank:typeof item.currentRank==="number"?item.currentRank:null,track:item.track,status:type,direction:item.action,judgment:item.judgment,visitors:item.visitors,conversion:item.conversion}}/>)}</div></Panel>
